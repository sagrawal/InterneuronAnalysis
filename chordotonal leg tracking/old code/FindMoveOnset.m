...
%movement. (1 and 2*framerate should work well)
%
...
%find the start of the tibia movement. (1.5*framerate should be enough).
%
...
function []=FindMoveOnset(InFile,MPW,MPD,LegAngleScale,FrameB,ThresP)
...
%will ignore first 50 frames of data, since often there is an initial
%movement artifact
...
%of the peak (PHeight).
[PKS,LOCS,WDTH,PHeight]=findpeaks(AbsDiffRealAngle(50:end),'MinPeakHeight',MinPeakHeight,'MinPeakWidth',MPW,'MinPeakDistance',MPD);
LOCS = LOCS + 50;
...
hold on
plot(RealAngle)
plot(StartVF, RealAngle(StartVF), 'ok')
plot(LOCS, RealAngle(LOCS), '.r')
position=strfind(InFile,'TrackLegAngle'); %gives the position of the period in the string FileName
title(InFile(1:position-1), 'Interpreter', 'none'); 

hold off

%Save the peak position, height, etc.
position=strfind(InFile,'.'); %gives the position of the period in the string FileName
NewName=InFile(1:position-15); 
Outfile = strcat(NewName,'FindMoveOnset');
