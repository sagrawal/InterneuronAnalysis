%% 12/3/18 edited to accomodate ephys rig data, will only be used for triangle wave data for now.
...
function []=FindWaveOnset(InFile,AngThresh, MPW,MPD,FrameB,ThresP)

%We will get the original leg angle from *TrackLegAngle4.mat file.
load(InFile);
RealAngle=double(legangles>AngThresh);%This is the angle. 
...
%Set the min peak height
% MinPeakHeight=0.5*max(AbsDiffRealAngle);
MinPeakHeight=nanstd(RealAngle);

figure;
% findpeaks(AbsDiffRealAngle,'MinPeakHeight',MinPeakHeight,'MinPeakWidth',MPW,'MinPeakDistance',MPD);
findpeaks(RealAngle,'MinPeakHeight',MinPeakHeight,'MinPeakWidth',MPW,'MinPeakDistance',MPD);
hold on
...
%of the peak (PHeight).
[PKS,LOCS,WDTH,PHeight]=findpeaks(RealAngle,'MinPeakHeight',MinPeakHeight,'MinPeakWidth',MPW,'MinPeakDistance',MPD);
length(LOCS)
...
Outfile = strcat(NewName,'_WithWaveOnset');
save(Outfile,'PKS','LOCS','currentinjectiondata', 'frame_on', 'FrameRate', 'legangles', 'movieFiles', 'SampleRate', 'stimcommanddata', 'voltagedata')
