...
dataDir = 'G:\My Drive\Sweta to backup\ephysdata\13B recordings\post atropine swings\';

fileTag = '13B_32*Onset.mat';
...
pre.swingstarts_frames = pre.LOCS(1:2:nswings*2);
% post.swingstarts_frames = post.LOCS(1:2:nswings*2);
...
for i = 1:nswings
    ...
    elseif (pre.swingstarts_frames(i)+(secafter*pre.FrameRate)) > length(pre.legangles)
        ...
        pre.offsetcorr(i) = mean(pre.swingstart_voltage(i, 1:(secbefore*pre.SampleRate)));
        pre.swingstart_voltage(i, :) = pre.swingstart_voltage(i, :) - pre.offsetcorr(i);
...
for i = 1:nswings
    ...
    elseif (post.swingstarts_frames(i)+(secafter*post.FrameRate)) > length(post.legangles)
        ...
        post.offsetcorr(i) = mean(post.swingstart_voltage(i, 1:(secbefore*post.SampleRate)));
        post.swingstart_voltage(i, :) = post.swingstart_voltage(i, :) - post.offsetcorr(i);
...
% export_fig(fig1,[dataDir fileTag(1:8), '_preMLAswings.pdf'], '-pdf','-nocrop', '-r600' , '-painters', '-rgb');
...
% export_fig(fig2,[dataDir fileTag(1:8), '_postMLAswings.pdf'], '-pdf','-nocrop', '-r600' , '-painters', '-rgb');
...
% export_fig(fig3,[dataDir fileTag(1:13), '_preandpostMLAswings.pdf'], '-pdf','-nocrop', '-r600' , '-painters', '-rgb');